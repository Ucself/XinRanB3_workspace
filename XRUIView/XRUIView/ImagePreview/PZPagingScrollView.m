...
- (void)logRect:(CGRect)rect withName:(NSString *)name {
    DBG_MSG(@"%@: %f, %f / %f, %f", name, rect.origin.x, rect.origin.y, rect.size.width, rect.size.height);
}

- (void)logLayout {
    DBG_MSG(@"#### PZPagingScrollView ###");
    
    [self logRect:self.bounds withName:@"self.bounds"];
    [self logRect:self.frame withName:@"self.frame"];
    
    DBG_MSG(@"contentSize: %f, %f", self.contentSize.width, self.contentSize.height);
    DBG_MSG(@"contentOffset: %f, %f", self.contentOffset.x, self.contentOffset.y);
    DBG_MSG(@"contentInset: %f, %f, %f, %f", self.contentInset.top, self.contentInset.right, self.contentInset.bottom, self.contentInset.left);
...
- (void)scrollViewDidEndDecelerating:(UIScrollView *)scrollView {
    ...
    if (self.pagingViewDelegate != nil) {
        [self.pagingViewDelegate pagingScrollView:self didPageViewForDisplay:nil forIndex:_currentPagingIndex];
    }
}
